...
const index = async (req, res) => {
    try {
        const content = JackpotBet.find(req.query).exec();
        res.json(content);
    } catch (error) {
        res.status(400).json(error);
    }
...
const show = async (req, res) => {
    try {
        const content = JackpotBet.findById(req.params.id).exec();
        res.json(content);
    } catch (error) {
        res.status(400).json(error);
    }
...
const update = async (req, res) => {
    try {
        const content = JackpotBet.findByIdAndUpdate(req.params.id, req.body).exec();
        res.json(content);
    } catch (error) {
        res.status(400).json(error);
    }
